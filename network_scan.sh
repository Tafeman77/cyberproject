...
nmap -sn -D --spoof-man 00:50:56:9D:E3:F9 -n "$TARGET_NETWORK" | grep "Nmap scan report for" | awk '{print $5}' | sort -u > "$LIVE_HOSTS"
...
if [ -s "$LIVE_HOSTS" ]; then
    ...
    cat "$LIVE_HOSTS" | xargs -P 5 -I {} nmap -n -D --spoof-man 00:50:56:9D:3C:C6 -sS -sV -O --host-timeout 5m {} -oN "$NMAP_OUTPUT_DIR/nmap_scan_{}.txt"
